...
int main(int argc, char** argv){
    ...
    map = cv::imread((std::string("/home/") +
		      std::string(user) + 
                      std::string("/catkin_ws/src/ISwaeguk/project2/src/ground_truth_map.pgm")).c_str(), CV_LOAD_IMAGE_GRAYSCALE);
    ...
    while(running){
	    ...
	    //initialise the first path point as current point to pursue
